/*
** quantum Cavity -- wrapper for a wave buffer
** Copyright (C) 2021-2025 Tactile Interactive, all rights reserved
*/

#include <stdexcept>

#include "../hilbert/qSpace.h"
#include "qCavity.h"
#include "../directAccessors.h"

int traceCreate = false;

/* ************************************************************ birth & death & basics */

// This produces a wave ready to hold an electron
qCavity::qCavity(qSpace *sp, qCx *useThisBuffer)
	: qBuffer() {

	magic = 'Cavi';

	if (! sp)
		throw std::runtime_error("qCavity::qCavity null space");
	magic = 'Wave';
	space = sp;

	if (traceCreate) {
		printf("🌊🌊 qCavity::qCavity(%s)  utb=%p => this=%p\n", space->label,
			useThisBuffer, this);
		printf("🌊🌊 qCavity::qCavity() wave's Space: %p  nPoints:%d\n", (space), space->nPoints);
		printf("      🌊🌊        qCavity: %p\n", (this));
	}

	initBuffer(space->nPoints, useThisBuffer);  // qBuffer method

	if (traceCreate)
		printf("      🌊🌊  allocated wave: %p\n", (wave));
	qDimension *dim = space->dimensions;
	nPoints = dim->nPoints;
	start = dim->start;
	end = dim->end;
	continuum = dim->continuum;

	if (traceCreate) {
		printf("🌊🌊 allocated qCavity::qCavity resulting qCavity obj: %p   sizeof qCavity = x%lx\n",
			this, (long) sizeof(qCavity));
		printf("        sizeof(int):%ld   sizeof(void *):%ld\n", sizeof(int), sizeof(void *));
	}

	FORMAT_DIRECT_OFFSETS;
}

qCavity::~qCavity(void) {
	// the qBuffer superclass frees the wave just like it allocates it

	if (traceCreate) {
		printf("🌊🌊 qCavity::~qCavity resulting qCavity obj: %p \n",
			this);
		printf("        sizeof(int):%ld   sizeof(void *):%ld\n", sizeof(int), sizeof(void *));
	}
}

	/* *********************************************** direct access */

// Insert this into the constructor and run this once.  Copy text output.
// Paste the output into class eCavity, the class itself, to replace the existing ones
void qCavity::formatDirectOffsets(void) {
	// don't need magic
	printf("🚦 🚦 ----------- starting    👽   👽    eCavity direct access    👽   👽    JS getters & setters ----------\n\n");

	makePointerGetter(wave);

	printf("\n");
	makeIntGetter(nPoints);
	makeIntGetter(start);
	makeIntGetter(end);
	makeIntGetter(continuum);

	printf("\n🚦 🚦 --------------- done with    👽   👽    eCavity direct access    👽   👽    --------------\n");
}


	/* *********************************************** iterators */


/* never tested - might never work  */
void qCavity::forEachPoint(void (*callback)(qCx, int) ) {
	qDimension *dims = space->dimensions;
	int end = dims->end + dims->start;
	qCx *wave = wave;
	for (int ix = 0; ix < end; ix++) {
		callback(wave[ix], ix);
	}
}

/* never tested - might never work   */
void qCavity::forEachState(void (*callback)(qCx, int) ) {
	qDimension *dims = space->dimensions;
	int end = dims->end;
	qCx *wave = wave;
	for (int ix = dims->start; ix < end; ix++) {
		callback(wave[ix], ix);
	}
}

/* ************************************************* bad ideas I might revisit?  */

double cleanOneNumber(double u, int ix, int sense) {
	if (!isfinite(u)) {
		// just enough to be nonzero without affecting the balance
		printf("🌊🌊 had to prune [%3d]= %f\n", ix, u);
		double faker = sense ? 1e-9 :  -1e-9;
		return faker;
	}
	return u;
}

// look for NaNs and other foul numbers, and replace them with something .. better.
void qCavity::prune() {
	printf("🌊🌊 Do we really have to do this?  let's stop.\n");

	qDimension *dims = space->dimensions;
	qCx *wave = wave;
	for (int ix = dims->start; ix < dims->end; ix++) {
		wave[ix].re = cleanOneNumber(wave[ix].re, ix, ix & 1);
		wave[ix].im = cleanOneNumber(wave[ix].im, ix, ix & 2);
	}
}

/* ******************************************************* C for JS */

qCavity *cavity_create(qSpace *space, qCx *useThisBuffer) {
	// useThisBuffer, usually null, haven't tested lately
	return new qCavity(space, useThisBuffer);
}

void cavity_delete(qCavity *qcavity) {
	delete qcavity;
}

// this will normalize with the C++ normalize
void cavity_normalize(qCavity *qcavity) {
	qcavity->normalize();
}

