...
wasmFileName = "quantumEngine.wasm";

if (!isDataURI(wasmFileName)) {
 wasmFileName = locateFile(wasmFileName);
}

function getBinary(file) {
 try {
  if (file == wasmFileName && wasmBinary) {
...
function instantiateArrayBuffer(binaryFile, imports, receiver) {
 return getBinaryPromise(binaryFile).then(function(binary) {
  ...
  if (isFileURI(wasmFileName)) {
   err("warning: Loading from a file URI (" + wasmFileName + ") is not supported in most browsers. See https://emscripten.org/docs/getting_started/FAQ.html#how-do-i-run-a-local-webserver-for-testing-why-does-my-program-stall-in-downloading-or-preparing");
  }
  abort(reason);
 });
}
...
function createWasm() {
 ...
 instantiateAsync(wasmBinary, wasmFileName, info, receiveInstantiationResult);
 return {};
}
