...
function handleSpacePromise(space, animator, context, setWVContext) {
	debugger;
	// this code don't know where it should go
	...
	if (animator && !animator.grinder)
		animator.grinder = this.grinder;

	// make room for the bumpers for WELL continuum (both sides) on the
	// WaveView.  Note that continuum can change only when page
	// reloads.  Where does this go?
	let bumperWidth = (qeConsts.contWELL == space.continuum)
		? WELL_BUMPER_WIDTH
		: 0;

	this.mainVDisp = space.vDisp;

	this.setUpContext(space, context, setWVContext);
}
